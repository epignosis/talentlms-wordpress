...
class Ajax implements PluginService{
	...
	public function register(): void{
		add_action('wp_ajax_tlms_resynch', array($this, 'tlms_resyncCourse'));
	}
	...
	public function tlms_resyncCourse(): void{
		...
		try{
			...
			if(!empty($courseId) && (empty($limit['remaining']) || $limit['remaining'] > 4)){ // we are gonna make at least 3 api calls, so we have to be prepared.
				...
				echo json_encode(array('api_limitation' => 'Api usage resets at '.$limit['formatted_reset']), JSON_THROW_ON_ERROR);
			}
		}
		catch(\Exception $exception){
			if($exception instanceof \InvalidArgumentException){
				echo json_encode(array('error' => 'Invalid course id'), JSON_THROW_ON_ERROR);
			}
		}
