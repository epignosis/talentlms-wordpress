...
class Utils
{
    ...
    public static function tlms_getDateFormat($no_sec = false)
    {
 // used in reg_shortcodes
        ...
        if (empty($result)) {
            ...
            foreach ($apiCourses as $course) {
                $wpdb->insert(TLMS_COURSES_TABLE, array(
                    ...
                    'category_id' => !empty($course['category_id']) ? (new TLMSPositiveInteger($course['category_id']))->getValue() : null,
                    ...
                    'hide_catalog' => (new TLMSInteger((int)$course['hide_from_catalog']))->getValue(),
                    'shared' => (new TLMSInteger((int)$course['shared']))->getValue(),
                    'shared_url' => !empty($course['shared_url']) ? (new TLMSUrl($course['shared_url']))->getValue(): null,
